...
class CMySQLV1 {
  ...
  async listTableObjects(req: Request, res: Response): Promise<void> {
    try {
      const result = await ListObjectsMySQLV1.listTableObjects(getConnectionKey(req), getQueryString(req, 'schema'));
      sendServiceResult(res, result);
    } catch (error: unknown) {
      sendInternalError(res, error);
    }
  ...
  async tableColumns(
    ...
    try {
      const result = await ListObjectsMySQLV1.tableColumns(
